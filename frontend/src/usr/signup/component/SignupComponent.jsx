    ...
    const handleVerifyCode = () => {
     ...
     const handleDuplicateCheck = async (field) => {
            try {
                ...
                if (!value) {
                    setAlertModalConfig({ isOpen: true, message: `${field === "userId" ? "아이디" : "닉네임"}를(을) 입력해주세요.` });
                    return;
                }
                ...
                if (response.data.isDuplicate) {
                    setAlertModalConfig({
                            isOpen: true,
                            message: `${value}은(는) 중복된 ${field === "userId" ? "아이디" : "닉네임"}입니다.`,
                        });
                    ...
                    setAlertModalConfig({
                            isOpen: true,
                            message: `${value}은(는) 사용 가능한 ${field === "userId" ? "아이디" : "닉네임"}입니다.`,
                        });
                    if (field === "userId") setIsUserIdChecked(true);
                    if (field === "nickname") setIsNicknameChecked(true);
                }
            } catch (error) {
                setAlertModalConfig({ isOpen: true, message: "중복 확인 중 문제가 발생했습니다." });
            }
        };
    ...
    const handleSubmit = async (e) => {
        ...
        if (!isEmailVerified) {
         ...
         if (!isNicknameChecked) {
                    setAlertModalConfig({ isOpen: true, message: "닉네임 중복 확인을 해주세요." });
                    return;
                }
        ...
        const errorFields = Object.keys(errors).filter((key) => errors[key]);
                if (errorFields.length > 0) {
                    ...
                    setAlertModalConfig({ isOpen: true, message: errors[firstErrorField], field: firstErrorField });
                    return;
                }
    ...
    return (
        <div>
            <form onSubmit={handleSubmit}>
                ...
                <div>
                    ...
                    <input
                        ...
                        onChange={handleChange}
                    />
                     <button type="button" onClick={() => handleDuplicateCheck("nickname")}>
                        중복 확인
                     </button>
            ...
            <AddressModal
                ...
                onComplete={(data) => {
                    setFormData({ ...formData, address: data.address, zonecode: data.zonecode });
